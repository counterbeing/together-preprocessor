...
function cleanup(cb) {
  src("photos/optimized", { read: false }).pipe(clean())
  cb()
}
...
const processMarkdown = md => {
  ;(converter = new showdown.Converter()), (html = converter.makeHtml(md))
  return html
}
...
async function renderStories() {
  ...
  const allPhotos = JSON.parse(
    fs.readFileSync("./src/components/photos.json")
  ).map(p => {
